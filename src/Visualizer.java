...
/**
 * Music visualizer scene based in a room with visuals on each side of the room
 * and a rgb floor. Balls can also be generated to float around the room or
 * bounce in time with the music. Music source defaults to the default
 * microphone, but clicking 5 loads the LonelyBoy.mp3 in the root directory. To
 * test the scene if microphone has not been set up.
 * 
...
public class Visualizer extends PApplet {
	...
	/**
	 * Must be fullscreen due to use of <em>java.awt.Robot</em> to keep mouse
	 * fixed in centre of screen for first person camera
	 * 
	 * @see java.awt.Robot
	 */
	...
	public void setup() {
		...
		for (int i = 0; i < (height * 0.02); i++) {
			...
			balls.add(new Ball(this, speed, radius, random(-height * 4, height * 4), random(0, -height * 4),
					random(-height * 4, height * 4), imgs[(int) random(0, 3)]));
		}
		...
		wavegraph = new ConcenticParticleWavegraph(this, new PVector(0, -height * 2, 0), height * 2 / 30, fft.avgSize(),
				3);
	...
	/**
	 * Switches between default microphone being used as audio source or the
	 * LonelyBoy.mp3 contained in the root directory
	 */
	...
	private void reshape() {
		objects.get("stage").size(height * 2); // Resize stage based on camera
												// radius
	...
	void lighting() {
		...
		ambientLight(ambience[0], ambience[1], ambience[2]); // set ambient
																// lighting in
																// the scene
		directionalLight(diffuse[0], diffuse[1], diffuse[2], // create
																// directional
																// diffuse
																// lighting
	...
	public void draw() {
		...
		// Adds light at top of stage that minics colour of the concentric
		// wavegraph
		...
		((ConcenticParticleWavegraph) objects.get("wavegraph")).scaleToResolution(new PVector(0, -height * 2, 0),
				height * 2 / 25);
		;
	...
	private void mouseMove(int wndWidth, int wndHeight) {
		try {
			...
			// Get the direction from the mouse cursor, set a resonable
			// maneuvering speed
