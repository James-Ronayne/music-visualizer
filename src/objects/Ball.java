...
public class Ball extends DisplayableObject {
	...
	/**
	 * Moves ball according to speed vector and gravity also does bounds
	 * checking
	 */
	public void move() {
		if (isGravity)
	...
	void initSphere(int numPtsW, int numPtsH_2pi) {
		...
		pointsHeight2PI = numPtsH_2pi; // How many actual pts around the sphere
										// (not just from top to bottom)
		pointsHeight = PApplet.ceil((float) pointsHeight2PI / 2) + 1; // How
																		// many
																		// pts
																		// from
																		// top
		...
		x = new float[pointsWidth]; // All the x-coor in a horizontal circle
									// radius 1
		y = new float[pointsHeight]; // All the y-coor in a vertical circle
										// radius 1
		z = new float[pointsWidth]; // All the z-coor in a horizontal circle
									// radius 1
		multXZ = new float[pointsHeight]; // The radius of each horizontal
											// circle (that you will multiply
											// with coorX and coorZ)

		for (int i = 0; i < pointsWidth; i++) { // For all the points around the
												// width
		...
		for (int i = 0; i < pointsHeight; i++) { // For all points from top to
													// bottom
			if ((int) (pointsHeight2PI / 2) != (float) pointsHeight2PI / 2 && i == pointsHeight - 1) { // If
																										// the
																										// numPointsH_2pi
																										// is
																										// odd
				...
				// The numPointsH_2pi and 2 below allows there to be a flat
				// bottom if the numPointsH is odd
				float thetaH = i * 2 * PApplet.PI / (pointsHeight2PI);

				// PI+ below makes the top always the point instead of the
				// bottom.
				y[i] = PApplet.cos(PApplet.PI + thetaH);
