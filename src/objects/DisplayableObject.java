...
public abstract class DisplayableObject {
    ...
    protected PVector pos = new PVector(0.f,0.f,0.f);                   // position in World Space
    protected PVector scale = new PVector(1.f,1.f,1.f);                 // size in World Space
    protected PVector rotation = new PVector(0.f,0.f,0.f);              // orientation in World Space
    
    public DisplayableObject(PApplet parent){ this.parent = parent; }   // Constructor
    ...
    public void size(float s){ size(s,s,s); }                           // set size in World Space
    public void size(float x, float y, float z){ scale.set(x,y,z); }    // set size in World Space
    
    public void orientation(float rx, float ry, float rz){ rotation.set(rx,ry,rz); }
    
    public PVector position(){ return pos; }                            // get position in World Space
    public PVector size(){ return scale; }                              // get size in World Space
    public PVector orientation(){ return rotation; }                    // get World Space orientation
